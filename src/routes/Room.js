...
const Room = (props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      socketRef.current = io("https://quickshare-backend.herokuapp.com/"); //Hosted socketIo server only use if you only want to make frontend changes
      ...
      worker.addEventListener("message", (e) =>
        down(e, fileNameRef.current, peerRef.current)
      );
  ...
  async function sendData(roomID, file, hostName, pubIp) {
    ...
    const response = await axios.post(
      "https://quickshare-backend.herokuapp.com/log",
      {
      ...
      <main>
        ...
        <div className="share-info">
          ...
          <div className="qrCont">
            <QRCode
              qrUrl={currentURL}
              params={props.match.params.roomID}
            ></QRCode>
          </div>
